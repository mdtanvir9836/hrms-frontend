import React, { useEffect } from "react";
import { TbUserHeart } from "react-icons/tb";
import { Formik, Form, Field, ErrorMessage } from "formik";
import * as Yup from "yup";
import {
  createContact,
  getContactById,
  updateContact,
} from "../../../redux/slice/ContactSlice";
import { useDispatch, useSelector } from "react-redux";
import { useParams } from "react-router-dom";

function ContactDetails() {
  const dispatch = useDispatch();
  const { loading, contacts } = useSelector((state) => state.contact);
  // console.log("contacts", contacts);
  const editId = useParams().id;
  console.log("editId", editId);
  const userId = editId? editId : localStorage.getItem("userId");

  useEffect(() => {
    if (userId) {
      dispatch(getContactById(userId));
    }
  }, [dispatch, userId]);

  // Form validation schema using Yup
  const validationSchema = Yup.object({
    contactName: Yup.string()
      .required("Name is required")
      .min(2, "Name must be at least 2 characters long"),
    contactNumber: Yup.string()
      .required("Phone number is required")
      .matches(/^[0-9]{10}$/, "Phone number must be 10 digits"),
  });

  // Initial form values
  const initialValues = {
    userId: userId,
    contactName: contacts?.contactName || "",
    contactNumber: contacts?.contactNumber || "",
  };
// console.log("initialValues", initialValues);

  // Form submission handler
  const handleSubmit = (values, { setSubmitting }) => {
    if (contacts?._id) {
      dispatch(updateContact({ id: contacts?._id, data: values })).unwrap();
    } else
      dispatch(createContact(values))
        .unwrap()
        .then(() => {
          // navigate("/name-setup");
          dispatch(getContactById(userId));
        })
        .catch((err) => {
          console.error("Submission error:", err);
        })
        .finally(() => {
          setSubmitting(false);
        });
  };

  // Render the form only if contacts are loaded

  return (
    <div class="JsOur jZmi-">
      <div class="LmTK2 _3k5ae">
        <div class="_6o1-A">
          <div class="WToIY medium XOGJ7">
            <div
              class="purg7v1 purg7vc7 purg7vd0 _15se5fi0"
              style={{ "--purg7v0": "transparent" }}
            >
              <div class="_6ILTG">
                <TbUserHeart size={30} color="#1F74EC" />
              </div>
            </div>
            <div class="Z2tZY">
              <span
                class="_6o3wap1 _6o3wapn _6o3wapt _6o3wap7 _6o3wap9"
                style={{ "--_6o3wap0": "var(--_1wf7mbl3s)" }}
              >
                Emergency contact
              </span>
            </div>
            <div class="lsgRP">
              <span
                class="_6o3wap1 _6o3wapm _6o3wapv _6o3wap7 _6o3wap9"
                style={{ "--_6o3wap0": "var(--_1wf7mbl3r)" }}
              >
                Fill in your emergency contact information.
              </span>
            </div>
          </div>
        </div>
        <div class="YI2-W">
          <div class="_6NtRo">
            <Formik
              initialValues={initialValues}
              validationSchema={validationSchema}
              enableReinitialize={true}
              onSubmit={handleSubmit}
            >
              {({ values, isSubmitting, setFieldValue }) => (
                <Form class="tY4kP">
                  {/* {console.log(values)} */}

                  <div class="OV-fI">
                    <label class="hqyye0">
                      <div
                        class="purg7v1 purg7vx purg7vc9 purg7vd6 _15se5fi0"
                        style={{ "--purg7v0": "transparent" }}
                      >
                        <div
                          class="purg7v1 purg7vc9 purg7vd5 _15se5fi0"
                          style={{ "--purg7v0": "transparent" }}
                        >
                          <div
                            class="purg7v1 purg7vc7 purg7vcf purg7vd8 _15se5fi0"
                            style={{ "--purg7v0": "transparent" }}
                          >
                            <span
                              class="_6o3wap1 _6o3wapl _6o3wapu _6o3wap7 _6o3wap9"
                              style={{ "--_6o3wap0": "var(--_1wf7mbl3s)" }}
                            >
                              Name
                            </span>
                          </div>
                        </div>
                        <div
                          class="purg7v1 purg7v5x purg7vc9 _15se5fio _15se5fi2 _15se5fi1v _1y2tv656 hqyye1"
                          style={{ "--purg7v0": "var(--_1wf7mbl3k)" }}
                        >
                          <Field
                            className="_1tr1dnd0"
                            type="text"
                            name="contactName"
                            value={values.contactName || contacts?.contactName} // Use short-circuit evaluation
                            onChange={(event) =>
                              setFieldValue("contactName", event.target.value)
                            } // Extract value from event
                          />
                        </div>
                        <ErrorMessage
                          name="contactName"
                          component="div"
                          className="text-red-500 text-sm"
                        />
                      </div>
                    </label>
                  </div>
                  <div class="OV-fI">
                    <label class="hqyye0">
                      <div
                        class="purg7v1 purg7vx purg7vc9 purg7vd6 _15se5fi0"
                        style={{ "--purg7v0": "transparent" }}
                      >
                        <div
                          class="purg7v1 purg7vc9 purg7vd5 _15se5fi0"
                          style={{ "--purg7v0": "transparent" }}
                        >
                          <div
                            class="purg7v1 purg7vc7 purg7vcf purg7vd8 _15se5fi0"
                            style={{ "--purg7v0": "transparent" }}
                          >
                            <span
                              class="_6o3wap1 _6o3wapl _6o3wapu _6o3wap7 _6o3wap9"
                              style={{ "--_6o3wap0": "var(--_1wf7mbl3s)" }}
                            >
                              Phone number
                            </span>
                          </div>
                        </div>
                        <div
                          class="purg7v1 purg7v5x purg7vc9 _15se5fio _15se5fi2 _15se5fi1v _1y2tv656 hqyye1"
                          style={{ "--purg7v0": "var(--_1wf7mbl3k)" }}
                        >
                          <Field
                            className="_1tr1dnd0"
                            type="text"
                            name="contactNumber"
                            value={
                              values.contactNumber || contacts?.contactNumber
                            } // Use short-circuit evaluation
                            onChange={(event) =>
                              setFieldValue("contactNumber", event.target.value)
                            } // Extract value from event
                          />
                        </div>
                        <ErrorMessage
                          name="contactNumber"
                          component="div"
                          className="text-red-500 text-sm"
                        />
                      </div>
                    </label>
                    <div class="OV-fI zyPqg mt-3">
                      <div class="OnRSf">
                        {values &&
                          (values.contactName || values.contactNumber) && (
                            <button
                              className="_1nz7aia3 _1nz7aia4"
                              type="submit"
                              disabled={isSubmitting || loading}
                              style={{
                                "--_1nz7aia2": "var(--_1wf7mbl52)",
                                "--_1nz7aia1": "var(--_1wf7mbl52)",
                                "--_1nz7aia0": "none",
                              }}
                            >
                              <span
                                className="_6o3wap1 _6o3wapl _6o3wapu _6o3wap7 _6o3wap9"
                                style={{ "--_6o3wap0": "var(--seicjt3d)" }}
                              >
                                Save
                              </span>
                            </button>
                          )}
                      </div>
                    </div>
                  </div>
                </Form>
              )}
            </Formik>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ContactDetails;
