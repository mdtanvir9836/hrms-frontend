...
import { fetchCategoryById } from "../../../../../redux/slice/documentCategorySlice";
...
function Agreement() {
  ...
  useEffect(() => {
    if (id || userId) {
      // dispatch(fetchEmploymentInfoByUserId(id));
      dispatch(fetchUserById(id || userId));
    }
  }, [id || userId, dispatch]);

  const { category } = useSelector((state) => state.documentCategory);
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {id && `${users.firstName}'s`} {category?.name}
                            </span>
                          </div>
                          <span
                          ...
                          >
                            <span
                              ...
                              style={{ "--_1sco1wt0": 5 }}
                            >
                              {category?._id === "678bb3e81715662eb100ef07"
                                ? id
                                  ? `These are all ${users.firstName}'s contracts.`
                                  : `${users.firstName}, your contract and other related documents are stored here.`
                                : category?._id === "678bb4371715662eb100ef18"
                                  ? id
                                    ? `All ${users.name}'s FMLA documents are stored here.`
                                    : `These are all the FMLA documents you've uploaded.`
                                  : category?._id === "678bb4451715662eb100ef1c"
                                    ? id
                                      ? `This is where  ${users.name}'s id documentation is stored.`
                                      : `Your ID and other personal documents are stored here.`
                                    : category?._id ===
                                        "678bb4561715662eb100ef20"
                                      ? id
                                        ? `This is where  ${users.name}'s documents are stored here.`
                                        : `Your documents are stored here.`
                                      : category?._id ===
                                            "678bb46b1715662eb100ef24" && id
                                        ? `This is ${users.name}'s payslip historic information.`
                                        : `Your payslips are stored here, review them each time you need them`}
                            </span>
                          </span>
                        </div>
                      </div>
